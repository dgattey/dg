 ...
 */
const fetchKey = async <Key extends EndpointKey>(key: Key): Promise<EndpointType<Key>> => {
 ...
 */
const useData = <Key extends EndpointKey>(key: Key) =>
  useSWR<AwaitedType<Key>, Error>(key, fetchKey);
