...
export function GlassContainer({ children, sx, ...props }: GlassContainerProps) {
  ...
  return (
      ...
      sx={mixinSx(
        [
          (theme) => {
            ...
            const glassColor = theme.vars.palette.grey[400];
            const lightColor = `color-mix(in srgb, ${theme.vars.palette.common.white} 80%, ${theme.vars.palette.common.black})`;
            const darkColor = `color-mix(in srgb, ${theme.vars.palette.common.black} 70%, ${theme.vars.palette.common.white})`;
            const lightReflex = theme.palette.mode === 'dark' ? 0.3 : 1;
            const darkReflex = theme.palette.mode === 'dark' ? 2 : 1;

            return {
              backdropFilter: `blur(8px) saturate(150%) url(#${filterId})`,
              backgroundColor: `color-mix(in srgb, ${glassColor} 12%, transparent)`,
              border: `1px solid color-mix(in srgb, ${lightColor} ${lightReflex * 10}%, transparent)`,
              borderRadius: theme.spacing(4),
              boxShadow: `
                inset 0 0 0 1px color-mix(in srgb, ${lightColor} ${lightReflex * 10}%, transparent),
                inset 1.8px 3px 0px -2px color-mix(in srgb, ${lightColor} ${lightReflex * 90}%, transparent),
                inset -2px -2px 0px -2px color-mix(in srgb, ${lightColor} ${lightReflex * 80}%, transparent),
                inset -3px -8px 1px -6px color-mix(in srgb, ${lightColor} ${lightReflex * 60}%, transparent),
                inset -0.3px -1px 4px 0px color-mix(in srgb, ${darkColor} ${darkReflex * 12}%, transparent),
                inset -1.5px 2.5px 0px -2px color-mix(in srgb, ${darkColor} ${darkReflex * 20}%, transparent),
                inset 0px 3px 4px -2px color-mix(in srgb, ${darkColor} ${darkReflex * 20}%, transparent),
                inset 2px -6.5px 1px -4px color-mix(in srgb, ${darkColor} ${darkReflex * 10}%, transparent),
                0px 1px 5px 0px color-mix(in srgb, ${darkColor} ${darkReflex * 10}%, transparent),
                0px 6px 16px 0px color-mix(in srgb, ${darkColor} ${darkReflex * 8}%, transparent)
                `,
              position: 'relative',
              willChange: 'backdrop-filter, background-color, border-color, box-shadow',
              ...bouncyTransition(
                theme,
                ['backdrop-filter', 'background-color', 'border-color', 'box-shadow'],
    ...
    >
      ...
      >
        <defs>
          <filter height="110%" id={filterId} primitiveUnits="objectBoundingBox">
            <feImage
              height="100%"
