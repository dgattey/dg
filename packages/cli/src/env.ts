...
import { fail, findRoot, fmt, out, spinner, withSpinner } from './lib/utils.js';
...
try {
  ...
  out(fmt.warn('1Password CLI not found, skipping .env generation'));
  process.exit(0);
}
...
if (!force && existsSync(envFile)) {
  ...
  if (existing === [...keys].sort().join('\n')) {
    out(fmt.success('.env already exists with correct keys'));
    process.exit(0);
  }
  out(fmt.info('.env keys mismatch, regenerating...'));
}
...
const secrets = await withSpinner(
  ...
  `Fetched ${keys.length} secrets from 1Password`,
);
